...
class ProductController extends Controller
{
    ...
    public function getOrders(Request $request)
    {
        ...
        if ($auth_user->roles->first() && $auth_user->roles->first()->name == 'admin') {
            ...
            $orders = Order::where('user_email', $auth_user->email)->paginate(10);
        }
    ...
    public function getPurchase(Request $request)
    {
        $auth_user = Auth::user();
        if ($auth_user->roles->first() && $auth_user->roles->first()->name == 'admin') {
            $purchaseOrders = Order::paginate(10);
        } else {
            $purchaseOrders = Order::where('user_email', $auth_user->email)->paginate(10);
        }
